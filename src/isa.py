...
# which is not the same as the field being zero. syntax is None when the
# operands are written in the same order the format places them.
Instr = namedtuple("Instr", "name fmt opcode funct3 funct7 syntax", defaults=(None, None, None))
...
INSTRS = [
    ...
    Instr("lw",    "I",   0x03, 0x2, syntax=("rd", "imm(rs1)")),
    Instr("jalr",  "I",   0x67, 0x0),
    Instr("sw",    "S",   0x23, 0x2, syntax=("rs2", "imm(rs1)")),
...
for entry in INSTRS:
    ...
    if entry.syntax is None:
        entry = entry._replace(syntax=FORMATS[entry.fmt])
    ...
    if entry.name in BY_NAME:
        raise ValueError(f"duplicate mnemonic {entry.name}")
